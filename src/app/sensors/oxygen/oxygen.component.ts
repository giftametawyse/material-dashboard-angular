...
export class OxygenComponent implements OnInit {
  ...
  ngOnInit(): void {
  this.http.get<any[]>('http://localhost:5000/api/oxymeter/history').subscribe({
      next: (data) => {
      ...
      error: (err) => {
        console.error('Failed to load oxygen data', err);
        this.oxygenData = [];
      }
    });
  }
  ...
  getFilteredData(): any[] {
    ...
    if (!query) return this.oxygenData || [];

    const targetDate = this.parseSearchDate(query);
    if (!targetDate) return [];

    return this.oxygenData.filter((row) =>
