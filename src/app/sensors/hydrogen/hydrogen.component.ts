...
export class HydrogenComponent implements OnInit,AfterViewInit {
  hydrogenData: any[] = [];
  searchQuery: string = '';
  ...
  ngOnInit(): void {
   this.http.get<any[]>('http://localhost:5000/api/hydrogen/history').subscribe({
      next: (data) => {
        this.hydrogenData = data;
  ...
  getFilteredData(): any[] {
    ...
    if (!query) return this.hydrogenData || [];

    const targetDate = this.parseSearchDate(query);
    if (!targetDate) return [];

    return this.hydrogenData.filter((row) =>
